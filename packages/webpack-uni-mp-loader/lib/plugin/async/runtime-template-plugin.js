...
class UniMpRuntimeTemplatePlugin {
  constructor (options) {
  ...
  apply (compiler) {
    ...
    compiler.hooks.compilation.tap({ name: pluginName }, compilation => {
      compilation.chunkTemplate.hooks.render.tap({ name: pluginName, stage: 1 }, (source, chunk) => {
        this.checkChunkDependencies(compilation, chunk)
        return source
      })

      compilation.hooks.optimizeChunkAssets.tapAsync(
        { name: pluginName, stage: -1 }, // -1, before 'terser-webpack-plugin'
...
module.exports = UniMpRuntimeTemplatePlugin
